...
class NotificationController extends GetxController {
  ...
  Future<void> _loadSavedState() async {
    try {
      ...
      print('📱 NotificationController: Loaded saved state - Updates: ${hasBookingUpdates.value}, Count: ${updateCount.value}');
    } catch (e) {
  ...
  Future<void> _saveState() async {
    try {
      ...
      print('💾 NotificationController: State saved - Updates: ${hasBookingUpdates.value}, Count: ${updateCount.value}');
    } catch (e) {
  ...
  Future<void> _checkForBookingUpdates() async {
    try {
      ...
      if (_lastKnownBookings.isEmpty) {
        ...
        print('📚 NotificationController: Initial bookings saved (${currentBookings.length} bookings)');
        return;
      }
      ...
      if (updates.isNotEmpty) {
        print('🔔 NotificationController: Found ${updates.length} booking updates!');
  ...
  List<String> _findBookingChanges(List<BookingModel> oldBookings, List<BookingModel> newBookings) {
    List<String> changes = [];
    
    // Check for new bookings
    for (BookingModel newBooking in newBookings) {
      final oldBooking = oldBookings.firstWhereOrNull((b) => b.id == newBooking.id);
      ...
      if (oldBooking == null) {
        ...
        if (oldBooking.status != newBooking.status) {
          changes.add('Booking ${newBooking.placeName ?? 'Unknown'}: ${oldBooking.status.name} → ${newBooking.status.name}');
        }
        
        // Check for payment status changes
        if (oldBooking.paymentStatus != newBooking.paymentStatus) {
          changes.add('Payment for ${newBooking.placeName ?? 'Unknown'}: ${oldBooking.paymentStatus} → ${newBooking.paymentStatus}');
        }
      }
    }
